...
```
注册/登录/刷新 → Supabase Auth → 返回 Token/用户信息
```
...
- 前端运行时错误与异常由 `@sentry/browser` 自动捕获。
- Worker 中的异常与关键事件通过 `console.error(JSON.stringify(...))` 输出。

**处理流程**

1. **前端日志**：
    - 在 `src/main.tsx` 或入口脚本中初始化 Sentry（已实现）。
    - 捕获全局错误与未处理 Promise Rejection 并发送到 Sentry。
2. **Worker 日志**：
    - 在 `src/workers.ts` 中捕获异常，使用 `console.error` 输出结构化日志。
    - Terraform 未配置 Logpush，则通过 Cloudflare Dashboard 查看日志，或在后续迭代中添加 Logpush Job。

**输出**

- Sentry 后端存储前端错误与性能数据。
- Cloudflare Dashboard 在线查看 Worker 日志。

**异常/边界处理**

- Sentry 网络故障时会缓存事件并重试发送。
- Worker 日志丢失无法补救，需后续补充 Logpush。

**简化链条**

```
前端错误 → Sentry.captureException → Sentry 后端；Worker 异常 → console.error → Cloudflare Dashboard
```
...
- 前端交易性能指标由 Sentry `tracesSampleRate` 上报。
- 后端 Worker 指标通过 Cloudflare Analytics 收集。

**处理流程**

1. **前端性能**：
    - Sentry SDK 自动采集页面加载与交互的性能指标。
    - 在 Sentry UI 对比各版本的数据。
2. **Worker 性能**：
    - Cloudflare Analytics 在 Dashboard 聚合请求延迟、错误率等。
    - 手动下载 Analytics CSV 或调用 API 生成报告。

**输出**

- Sentry Performance Tab 展示前端 Trace。
- Cloudflare Analytics Dashboard 展示 Worker 指标。

**异常/边界处理**

- 指标缺失：需在 Terraform 或后续迭代中配置 Metrics Exporter。

**简化链条**
``Sentry SDK → Performance 上报；Cloudflare Analytics → 延迟/错误率展示``
...
- Cloudflare 未配置 Alert Rule，但可在 Dashboard 手动创建告警。
- Sentry Alert 配置通过 Web UI（非代码化）。

**处理流程**

1. **Sentry 告警**：
   - 在 Sentry UI 设置错误率或性能阈值告警，触发邮件/Slack 通知。
2. **Cloudflare 告警**：
   - 在 Dashboard 手动创建阈值阈值规则（如 5xx >1%），发送到 Slack Webhook。

**输出**

- 邮件或 Slack 实时通知开发/运维团队。

**异常/边界处理**

- 通知失败：需在 Sentry 或 Cloudflare UI 中监控告警发送状态。

**简化链条**
``错误率超阈值 → Sentry/Cloudflare 触发 → 通知渠道``
...
- `main` 分支有 Push 事件触发 GitHub Actions 工具链。

**处理流程**

1. **构建与测试**：
   - Checkout 代码、安装依赖。
   - 运行 `npm run validate:schema`、`npm run build`。
   - （缺少 Playwright E2E 与部署步骤）。
2. **部署**：
   - 暂未配置 `wrangler publish` 或 `terraform apply` 步骤，需手动执行。
3. **回滚**：
   - 通过 Cloudflare Pages Atomic Deploy 功能手动回滚。

**输出**

- 成功构建产物存入 `dist`。
- 代码尚未自动部署至 Cloudflare workers/pages。

**异常/边界处理**

- 构建失败：GitHub Actions 标红并阻止合并。
- 部署未自动执行：需人工干预。

**简化链条**
``Push → CI 验证 → 构建产物；部署步骤待补``
